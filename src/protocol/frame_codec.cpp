 ...
 */
auto EncodeMessage(const ProtocolRequest &req, const ProtocolLimits &limits) -> std::string {
  RpcRequestHeader pb_hdr;
  pb_hdr.set_service_name(req.service_name_);
  pb_hdr.set_method_name(req.method_name_);
  ...
  hdr.request_id_ = req.request_id_;
  hdr.header_len_ = static_cast<uint32_t>(header_bytes.size());
  hdr.payload_len_ = static_cast<uint32_t>(req.payload_.size());

  return BuildFrame(hdr, header_bytes, req.payload_, limits);
}
 ...
 */
auto EncodeResponseHeader(const ProtocolResponse &resp) -> std::string {
  if (resp.error_code_ == 0 && resp.error_text_.empty()) {
  ...
  pb_hdr.set_error_code(resp.error_code_);
  pb_hdr.set_error_text(resp.error_text_);
 ...
 */
auto EncodeMessage(const ProtocolResponse &resp, const ProtocolLimits &limits) -> std::string {
  std::string header_bytes = EncodeResponseHeader(resp);

  FixedHeader hdr;
  hdr.message_type_ = MessageType::Response;
  hdr.request_id_ = resp.request_id_;
  hdr.header_len_ = static_cast<uint32_t>(header_bytes.size());
  hdr.payload_len_ = static_cast<uint32_t>(resp.payload_.size());

  return BuildFrame(hdr, header_bytes, resp.payload_, limits);
}
 ...
 */
auto DecodeProtocolRequest(const FixedHeader &hdr, std::string_view header_bytes, std::string_view payload,
                           RequestHeaderDecodeCache &cache) -> std::optional<ProtocolRequest> {
  ...
  std::optional<ProtocolRequest> decoded = DecodeProtocolRequest(hdr, header_bytes, payload);
  if (!decoded) {
    return std::nullopt;
  }
...
/**
 * @brief Decodes response metadata and payload.
 */
auto DecodeResponse(const FixedHeader &hdr, std::string_view header_bytes, std::string_view payload)
  ...
  resp.error_code_ = pb_hdr.error_code();
  resp.error_text_ = pb_hdr.error_text();
 ...
 */
auto FrameCodec::EncodeRequest(const ProtocolRequest &req) -> std::string { return EncodeMessage(req, limits_); }

/**
 * @brief Encodes one response frame with this codec's limits.
 */
auto FrameCodec::EncodeResponse(const ProtocolResponse &resp) -> std::string { return EncodeMessage(resp, limits_); }
...
auto FrameCodec::TryDecodeRequest(std::string_view buf, RequestHeaderDecodeCache &request_header_cache)
  ...
  std::optional<ProtocolRequest> request =
      DecodeProtocolRequest(frame.header_, frame.header_bytes_, frame.payload_, request_header_cache);
  if (!request) {
  ...
  switch (frame.header_.message_type_) {
    case MessageType::Request: {
      ...
      if (request_header_cache != nullptr) {
        request = DecodeProtocolRequest(frame.header_, frame.header_bytes_, frame.payload_, *request_header_cache);
      } else {
        request = DecodeProtocolRequest(frame.header_, frame.header_bytes_, frame.payload_);
      }
      if (!request) {
    ...
    case MessageType::Response: {
      std::optional<ProtocolResponse> response = DecodeResponse(frame.header_, frame.header_bytes_, frame.payload_);
      if (!response) {
        return {.error_ = ProtocolError::DecodeError, .consumed_ = frame.consumed_};
      }
